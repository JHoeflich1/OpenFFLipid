...
correct_length = msd_data.shape[1] == 5001

if correct_length:
    print("All rows have 5001 entries.")
else:
    print("Some rows do not have 5001 entries.")
...
for idx, row in msd_data.iterrows():
    if len(row) != 5001:
        print(f"Row {idx} does not have 5001 entries. It has {len(row)} entries.")
...
if all_numeric_and_no_nan:
    print("All rows have 5001 numerical entries with no NaN values.")
else:
