...
file = 'bootstrapped_Ds.pkl'
msd_df = pd.read_pickle(file)
print(msd_df.head())
msd_df.to_csv('bootstrapped_Ds.csv')
...
# # Check if all rows have 2001 columns
# import pandas as pd

# # Load the DataFrame
# msd_data = pd.read_pickle('msd_data_new.pkl')

# # Check if all rows have 2001 columns and no NaN values
# correct_length = msd_data.shape[1] == 2001

# if correct_length:
#     print("All rows have 2001 entries.")
# else:
#     print("Some rows do not have 2001 entries.")
...
#     if len(row) != 2001:
#         print(f"Row {idx} does not have 2001 entries. It has {len(row)} entries.")
...
#     print("All rows have 2001 numerical entries with no NaN values.")
# else:
